...
inline void store_be32(uint32_t x, uint8_t* out) {
...
inline void compress_block(uint32_t state[8], const uint8_t block[64]) {
    uint32_t w[64];
...
inline void compress_two_to_one_raw(
...
inline std::array<uint8_t, 32> compress_two_to_one(
...
class Sha256Ctx {
public:
    Sha256Ctx() : total_len_(0), buf_len_(0) {
        for (size_t i = 0; i < 8; ++i) {
