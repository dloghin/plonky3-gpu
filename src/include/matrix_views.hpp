...
public:
  RowIndexMappedView(const Matrix<T> &inner, size_t mapped_height,
                     RowMapper row_mapper)
      : inner_(inner), mapped_height_(mapped_height),
        row_mapper_(std::move(row_mapper)) {}
...
template <typename T> class VerticallyStridedMatrixView : public Matrix<T> {
public:
  VerticallyStridedMatrixView(const Matrix<T> &inner, size_t stride,
                              size_t offset)
      : inner_(inner), stride_(stride), offset_(offset),
        height_(compute_height(inner.height(), stride, offset)) {
    if (stride_ == 0) {
      throw std::invalid_argument(
          "VerticallyStridedMatrixView: stride must be > 0");
    }
  }
...
private:
  static size_t compute_height(size_t inner_height, size_t stride,
                               size_t offset) {
    if (stride == 0 || offset >= inner_height) {
      return 0;
    }
    return (inner_height - offset + stride - 1) / stride;
  }
...
template <typename T> class BitReversedMatrixView : public Matrix<T> {
public:
...
template <typename T> class HorizontallyTruncatedView : public Matrix<T> {
public:
  HorizontallyTruncatedView(const Matrix<T> &inner, size_t truncated_width)
    ...
    if (truncated_width > inner.width()) {
      throw std::invalid_argument(
          "HorizontallyTruncatedView: truncated width exceeds inner width");
    }
  }
  ...
  const T *row_ptr(size_t r) const override { return inner_.row_ptr(r); }
...
template <typename T> class VerticallyStackedMatrices : public Matrix<T> {
public:
  // Performance note:
  // - get_unchecked/get do a row-to-submatrix translation via locate_row() on
  // each call.
  // - That lookup is O(log num_stacked_matrices) because it uses binary search.
  // For row-wise processing, prefer row_ptr(r) when available, since it
  // performs the row lookup once and then allows direct contiguous access to
  // that row.
  explicit VerticallyStackedMatrices(
      std::vector<std::reference_wrapper<const Matrix<T>>> matrices)
    ...
    for (const auto &matrix_ref : matrices_) {
      ...
      if (m.width() != width_) {
        throw std::invalid_argument(
            "VerticallyStackedMatrices: all matrices must have equal width");
      }
...
private:
  std::pair<size_t, size_t> locate_row(size_t r) const {
    ...
    size_t matrix_idx =
        static_cast<size_t>(std::distance(start_rows_.begin(), it));
    if (matrix_idx > 0) {
      --matrix_idx;
    }
