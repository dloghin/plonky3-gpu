...
/**
 * Poseidon2 for BN254 (S-box degree 5, WIDTH=3), matching p3-bn254.
 *
 * The internal diffusion matrix for WIDTH=3 is:
 *   [2 1 1]
 *   [1 2 1]
 *   [1 1 3]
 ...
 *   ROUNDS_F = 8 (4 initial + 4 terminal external rounds)
 *   ROUNDS_P = 56 partial (internal) rounds
 */
...
using p3_field::Bn254;

constexpr uint64_t BN254_S_BOX_DEGREE = 5;
constexpr size_t BN254_WIDTH = 3;
constexpr size_t BN254_HALF_FULL_ROUNDS = 4;
constexpr size_t BN254_PARTIAL_ROUNDS = 56;

// ---------------------------------------------------------------------------
// Internal layer: custom matmul for WIDTH=3
//
//   [2 1 1]       [1 0 0]
//   [1 2 1] = I + [0 0 0] + ones_matrix
//   [1 1 3]       [0 0 1]
//
// Optimized as:  sum = s0 + s1 + s2;  s0 += sum;  s1 += sum;  s2 = 2*s2 + sum;
// ---------------------------------------------------------------------------

inline void bn254_matmul_internal(std::array<Bn254, 3>& state) {
    // Bracket so that sum can be computed before the s-box on state[0] finishes
    Bn254 sum = state[0] + (state[1] + state[2]);
    state[0] = state[0] + sum;
    state[1] = state[1] + sum;
    state[2] = state[2].double_val() + sum;
}

class Bn254Poseidon2Internal : public InternalLayer<Bn254, BN254_WIDTH, BN254_S_BOX_DEGREE> {
    std::vector<Bn254> constants_;

public:
    explicit Bn254Poseidon2Internal(std::vector<Bn254> constants)
        : constants_(std::move(constants)) {}

    void permute_state(std::array<Bn254, BN254_WIDTH>& state) override {
        for (const auto& rc : constants_) {
            state[0] = state[0] + rc;
            state[0] = state[0].template injective_exp_n<BN254_S_BOX_DEGREE>();
            bn254_matmul_internal(state);
        }
    }
};

// ---------------------------------------------------------------------------
// Round constants (canonical form — converted to Montgomery on construction)
//
// Extracted from zkhash poseidon2_instance_bn256 RC3 table.
// ---------------------------------------------------------------------------

inline Bn254 bn254_from_canonical_limbs(uint64_t l0, uint64_t l1, uint64_t l2, uint64_t l3) {
    uint64_t limbs[4] = {l0, l1, l2, l3};
    return Bn254::from_canonical(limbs);
}

inline std::vector<std::array<Bn254, 3>> bn254_poseidon2_external_initial() {
    // 4 rounds x 3 elements
    static const uint64_t D[4][3][4] = {
    ...
    std::vector<std::array<Bn254, 3>> v;
    v.reserve(4);
    for (const auto& row : D) {
        std::array<Bn254, 3> a;
        for (size_t i = 0; i < 3; ++i) {
            a[i] = bn254_from_canonical_limbs(row[i][0], row[i][1], row[i][2], row[i][3]);
        }
        v.push_back(a);
    }
    return v;
...
inline std::vector<std::array<Bn254, 3>> bn254_poseidon2_external_terminal() {
    static const uint64_t D[4][3][4] = {
...
inline std::vector<Bn254> bn254_poseidon2_internal_constants() {
    static const uint64_t D[56][4] = {
    ...
    v.reserve(56);
    for (const auto& row : D) {
        v.push_back(bn254_from_canonical_limbs(row[0], row[1], row[2], row[3]));
    }
    return v;
}

// ---------------------------------------------------------------------------
// External layer: uses HLMDSMat4 (same as Rust reference)
// For WIDTH=3, mds_light_permutation simply does state[i] += sum.
// ---------------------------------------------------------------------------

class Bn254Poseidon2External : public ExternalLayer<Bn254, BN254_WIDTH, BN254_S_BOX_DEGREE> {
    ExternalLayerConstants<Bn254, BN254_WIDTH> constants_;
    HLMDSMat4<Bn254> mds_mat_;

public:
    explicit Bn254Poseidon2External(ExternalLayerConstants<Bn254, BN254_WIDTH> constants)
        : constants_(std::move(constants)) {}

    void permute_state_initial(std::array<Bn254, BN254_WIDTH>& state) override {
        external_initial_permute_state<Bn254, Bn254, BN254_WIDTH>(
            state,
            constants_.get_initial_constants(),
            [](Bn254& val, const Bn254& rc) {
                val = val + rc;
                val = val.template injective_exp_n<BN254_S_BOX_DEGREE>();
            },
            &mds_mat_
        );
    }

    void permute_state_terminal(std::array<Bn254, BN254_WIDTH>& state) override {
        external_terminal_permute_state<Bn254, Bn254, BN254_WIDTH>(
            state,
...
inline std::shared_ptr<Poseidon2<Bn254, Bn254, BN254_WIDTH, BN254_S_BOX_DEGREE>>
default_bn254_poseidon2() {
    ExternalLayerConstants<Bn254, BN254_WIDTH> ext(
        bn254_poseidon2_external_initial(),
        bn254_poseidon2_external_terminal()
    );
    auto ext_layer = std::make_shared<Bn254Poseidon2External>(std::move(ext));
    auto int_layer = std::make_shared<Bn254Poseidon2Internal>(bn254_poseidon2_internal_constants());
    return std::make_shared<Poseidon2<Bn254, Bn254, BN254_WIDTH, BN254_S_BOX_DEGREE>>(
        ext_layer, int_layer
    );
}
