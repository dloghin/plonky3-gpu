...
// ---------------------------------------------------------------------------
// Create a Poseidon2Wrapper that the CPU PaddingFreeSponge /
// TruncatedPermutation expect (expose permute_mut(std::array<BB,WIDTH>&)).
// ---------------------------------------------------------------------------
...
using CPUSponge =
    p3_symmetric::PaddingFreeSponge<CPUPermWrapper, BB, WIDTH, RATE, DIGEST>;
using CPUTrunc =
    p3_symmetric::TruncatedPermutation<CPUPermWrapper, BB, 2, DIGEST, WIDTH>;
...
static bool test_cuda_matches_cpu(const char *label,
                                  ...
                                  const PosType &poseidon, size_t cap_height) {
  ...
  MerkleTree<BB, BB, DIGEST> gpu_tree =
      build_merkle_tree_cuda<BB, WIDTH, RATE, DIGEST, ROUNDS_F, ROUNDS_P,
                             D_EXP>(std::move(matrices_cuda), poseidon,
                                    cap_height);
...
static bool test_openings(const char *label,
  ...
  MerkleTree<BB, BB, DIGEST> gpu_tree =
      build_merkle_tree_cuda<BB, WIDTH, RATE, DIGEST, ROUNDS_F, ROUNDS_P,
                             D_EXP>(std::move(matrices), poseidon, cap_height);
  MerkleCap<BB, DIGEST> gpu_cap = get_cap(gpu_tree);
  ...
  for (size_t i = 0; i < max_height; ++i) {
    ...
    MerkleTreeMmcs<BB, BB, CPUSponge, CPUTrunc, DIGEST> verify_mmcs(
        CPUSponge{CPUPermWrapper{poseidon}}, CPUTrunc{CPUPermWrapper{poseidon}},
        cap_height);
...
// Performance test
// ---------------------------------------------------------------------------
static void perf_test(size_t num_rows, size_t num_cols,
...
int main() {
  ...
  auto run_match = [&](const char *label, std::vector<RowMajorMatrix<BB>> mats,
                       size_t cap_height) {
    // Make a copy for CPU
  ...
  // Single row
  run_match("single row (1x3)", {make_random_matrix(1, 3, rng)}, 0);

  // Single matrix, small
  run_match("single matrix (4x3)", {make_random_matrix(4, 3, rng)}, 0);

  // Single matrix, cap_height=0
  run_match("single matrix (8x2, cap=0)", {make_random_matrix(8, 2, rng)}, 0);

  // Single matrix, cap_height=2
  run_match("single matrix (8x4, cap=2)", {make_random_matrix(8, 4, rng)}, 2);

  // Single matrix, 16 columns (FRI leaf case)
  run_match("single matrix (16x16, cap=0)", {make_random_matrix(16, 16, rng)},
            0);

  // Two matrices, same height
  run_match("two matrices same height (4x2 + 4x3)",
            {make_random_matrix(4, 2, rng), make_random_matrix(4, 3, rng)}, 0);

  // Two matrices, mixed height
  run_match("mixed height (8x2 tall + 4x3 short)",
            {make_random_matrix(8, 2, rng), make_random_matrix(4, 3, rng)}, 0);

  // Three matrices, mixed height
  run_match("three matrices mixed (16x2 + 8x3 + 4x2)",
            {make_random_matrix(16, 2, rng), make_random_matrix(8, 3, rng),
             make_random_matrix(4, 2, rng)},
            0);
  ...
  run_match("large matrix (1024x8, cap=3)", {make_random_matrix(1024, 8, rng)},
            3);
  ...
  auto run_open = [&](const char *label, std::vector<RowMajorMatrix<BB>> mats,
                      size_t cap_height) {
    ++total;
