...
public:
    ...
    P3_HOST static const RoundConstants& round_constants() {
        static const RoundConstants constants = []() {
            RoundConstants out{};
            constexpr std::array<std::array<uint32_t, WIDTH>, NUM_FULL_ROUNDS> kRoundConstantsU32 = {{
            ...
            for (size_t r = 0; r < NUM_FULL_ROUNDS; ++r) {
                for (size_t i = 0; i < WIDTH; ++i) {
                    out[r][i] = Mersenne31(kRoundConstantsU32[r][i]);
                }
            }
            return out;
        }();
...
private:
    ...
    P3_HOST_DEVICE static void add_round_constants(State& state, size_t round_idx) {
        ...
        for (size_t i = 0; i < WIDTH; ++i) {
            state[i] += Mersenne31(kRoundConstantsU32[round_idx][i]);
        }
    }
